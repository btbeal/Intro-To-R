...
Now, answer the following questions.
  ...
  4. Remove the first value from the vector `new_vec` (recall that we can use a '-' sign to remove values) and store it in `new_vec2`
...
Using the `iris` data set, and our dplyr verbs, do the following...
  ...
  2. Using our `filter()` command, filter out all values less than 2.3 and less than 4. How many rows are in that data set?
